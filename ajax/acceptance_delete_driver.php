 ...
 * CHECKING DATA
 */
$q_check = "SELECT `driver_id` FROM `t_truckmixer` WHERE `driver_id` = '".$_POST['id_driver']."'";
$r_check = $mysqli->query($q_check);
if(mysqli_num_rows($r_check) == 0){
    $output = array(
        "status"    =>  0,
        "msg"       =>  "data <b>".$_POST['id_driver']."</b> sudah dihapus, silahkan reload halaman ini"
    );
    exit(json_encode($output));
}
...
$q_del = "DELETE FROM `t_truckmixer` WHERE `driver_id` = '".$_POST['id_driver']."'";
...
$output = array(
    "status"    =>  1,
    "msg"       =>  "Driver ID : ".$_POST['id_driver']." berhasil dihapus, halaman akan direfresh"
);
exit(json_encode($output));
