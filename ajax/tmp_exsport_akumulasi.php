...
if(IS_AJAX){
	...
	$mysqli = mysqli_connect(DB_SERVER,DB_USER,DB_PASS);
        ...
        if(!$mysqli){
            throw new Exception("<b>Error001</b> : ".  mysql_error($active_record));
        }
        if(!mysqli_select_db($mysqli,DB_NAME)){throw new Exception("<b>Error002</b> : ".  mysqli_error($active_record));}
        if(empty($_POST['delvdate']) ){
            throw new Exception("Kolom <b>Tanggal </b> Harus diisi!");
        }
        extract($_POST);
        $trunc = mysqli_query($mysqli,"TRUNCATE TABLE `tmp_exsport_akumulasi`");
        ...
        foreach ($result as $bt){
            ...
            $save_status = $active_record->insert("tmp_exsport_akumulasi", $tambah);
            if(!$save_status){
                $output = array(
                                                    "status"    => 0,
                                                    "msg"       =>  'error00'
                        ...
                        $cari_so = $active_record->query($so);
                            foreach ($cari_so as $c_s){
                                $sono_ad              =  $c_s['so_no'];
                                    ...
                                    $save_status_adj = $active_record->insert("tmp_exsport_akumulasi", $tambah_adj);
                                    if(!$save_status_adj){
...
//                            print_r($p_k);
//                            print_r($edit_adj);
                            $q_update = "UPDATE `tmp_exsport_akumulasi` SET `actual_qty` = '$akum' WHERE `docket_no` = '$docketno' AND `material_code` = '$materialcode' ";
//                            print_r($q_update);
