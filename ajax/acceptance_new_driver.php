...
if($mis->special_char($_POST['flatno']));
 ...
 * CHECKING DB
 */
// $q_check = "SELECT `no_pol` FROM `t_truckmixer` WHERE `no_pol` = '".$_POST['flatno']."'";
// $r_check = $mysqli->query($q_check);
// if(mysqli_num_rows($r_check) > 0){
//     $output = array(
//         "status"    =>  0,
//         "msg"       =>  "Plat Nomor <b>".$_POST['flatno']."</b> sudah digunakan"
//     );
//     exit(json_encode($output));
// }
...
$q_save = "INSERT INTO `t_truckmixer` "
        . "(`no_pol`,`driver_name`, `no_telp`)  "
        . "VALUES (   '".$_POST['flatno']."',   "
        . "'".$_POST['namaDriver']."',   "
        . "'".$_POST['noTelp']."' );";
...
$output = array(
    "status"    =>  1,
    "msg"       =>  "Plat Nomor Kendaraan baru berhasil disimpan, halaman akan di-refresh"
);
exit(json_encode($output));
