...
//        $excel->getActiveSheet()->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $excel->getActiveSheet()->setCellValue('X3', "0002-100442");
        $excel->getActiveSheet()->setCellValue('X4', "FLOBAS RPF-34");
        ...
        $excel->getActiveSheet()->setCellValue('AO3', "0002-100442");
        $excel->getActiveSheet()->setCellValue('AO4', "FLOBAS RPF-34");
        ...
        while($obj = $result->fetch_object()){
            ...
            $query_select_total = "SELECT `material_code`,`design_qty`,`actual_qty`,`target_qty` FROM `batch_transaction`,`tmp_batch_transaction_detail`
                                    WHERE `batch_transaction`.`docket_no` = `tmp_batch_transaction_detail`.`docket_no`
            ...
            if($result_total = $mysqli->query($query_select_total)){
                ...
                while($obj_total = $result_total->fetch_object()){
                    ...
                    elseif($obj_total->material_code == "0002-100442"){ //flobas rpf34
                        $act_total_flobasrpf34+=$obj_total->actual_qty;
                        $tar_total_flobasrpf34+=$obj_total->target_qty;
                        $excel->getActiveSheet()->setCellValue('X5', $act_total_flobasrpf34);
                        $excel->getActiveSheet()->setCellValue('AO5', $tar_total_flobasrpf34);
                    }
            ...
            $query_adj = "SELECT batch_transaction.`docket_no`,`batch_transaction`.`so_no`,`material_code`,`design_qty`,`actual_qty`,`target_qty` 
                          FROM `batch_transaction`,`tmp_batch_transaction_detail`
                          WHERE `batch_transaction`.`docket_no` = `tmp_batch_transaction_detail`.`docket_no`
                          AND `batch_transaction`.`delv_date` = '".$obj->delv_date."'";
            //echo $query_adj; exit();
            if($result_detail_adj = $mysqli->query($query_adj)){
                ...
                $docket = 0;
                //$so = $result_detail_adj->so_no;
//                if($docket_no == $so){ //sand / pasir
...
//                    }
            ...
            $query_select_detail = "SELECT * "
                    . "FROM `tmp_batch_transaction_detail` "
            ...
            if($result_detail = $mysqli->query($query_select_detail)){
                ...
                while($obj_detail = $result_detail->fetch_object()){     
                    ...
                    if($obj_detail->material_code == "0000-100001"){ //sand / pasir
                        ...
                        $excel->getActiveSheet()->setCellValue("O".$row,  $act_qty_sand);
                        $excel->getActiveSheet()->setCellValue('AF'.$row, $tar_qty_sand);
                        $excel->getActiveSheet()->setCellValue('AW'.$row, $moi_qty_sand);
                    }
                    elseif($obj_detail->material_code == "0000-100002"){ //MSand
                        ...
                        $excel->getActiveSheet()->setCellValue("P".$row, $act_qty_msand);
                        $excel->getActiveSheet()->setCellValue('AG'.$row, $tar_qty_msand);
                        $excel->getActiveSheet()->setCellValue('AX'.$row, $moi_qty_msand);
                    ...
                    elseif($obj_detail->material_code == "0000-100005"){ //AGG1 
                        ...
                        $excel->getActiveSheet()->setCellValue("Q".$row, $act_qty_agg1);
                        $excel->getActiveSheet()->setCellValue('AH'.$row, $tar_qty_agg1);
                        $excel->getActiveSheet()->setCellValue('AY'.$row, $moi_qty_agg1);
                    }
                    elseif($obj_detail->material_code == "0000-100006"){ //AGG2
                        ...
                        $excel->getActiveSheet()->setCellValue("R".$row,  $act_qty_agg2);
                        $excel->getActiveSheet()->setCellValue('AI'.$row, $tar_qty_agg2);
                        $excel->getActiveSheet()->setCellValue('AZ'.$row, $moi_qty_agg2);
                    ...
                    elseif($obj_detail->material_code == "0000-100007"){ //AGG3
                        ...
                        $excel->getActiveSheet()->setCellValue("S".$row, $act_qty_agg3);
                        $excel->getActiveSheet()->setCellValue("AJ".$row, $tar_qty_agg3);
                        $excel->getActiveSheet()->setCellValue('BA'.$row, $moi_qty_agg3);
                        
                    }
                    elseif($obj_detail->material_code == "0000-100050"){ //LMS
                        $act_qty_lms+=$obj_detail->actual_qty;
                        $tar_qty_lms+=$obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("T".$row, $act_qty_lms);
                        $excel->getActiveSheet()->setCellValue("AK".$row, $tar_qty_lms);
                    ...
                    elseif($obj_detail->material_code == "0000-100011"){ //Cement 
                        ...
                        $excel->getActiveSheet()->setCellValue("U".$row, $act_qty_cement);
                        $excel->getActiveSheet()->setCellValue("AL".$row, $tar_qty_cement);
                        
                    }
                    elseif($obj_detail->material_code == "0000-000015"){ //Flying Ash
                        $act_qty_fa+= $obj_detail->actual_qty;
                        $tar_qty_fa+= $obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("V".$row, $act_qty_fa);
                        $excel->getActiveSheet()->setCellValue("AM".$row, $tar_qty_fa);
                    ...
                    elseif($obj_detail->material_code == "0000-100030"){ // water / Air
                        ...
                        $excel->getActiveSheet()->setCellValue("W".$row, $act_qty_water);
                        $excel->getActiveSheet()->setCellValue("AN".$row, $tar_qty_water);
                    ...
                    elseif($obj_detail->material_code == "0002-100442"){ //Flobas Rpf34
                        ...
                        $excel->getActiveSheet()->setCellValue("X".$row, $act_qty_flobasrpf34);
                        $excel->getActiveSheet()->setCellValue("AO".$row, $tar_qty_flobasrpf34);         
                    }
                    elseif($obj_detail->material_code == "0002-100201"){ //523 N 
                        $act_qty_s523n+=$obj_detail->actual_qty;
                        $tar_qty_s523n+=$obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("Y".$row, $act_qty_s523n);
                        $excel->getActiveSheet()->setCellValue("AP".$row, $tar_qty_s523n);
                        
                    }
                    elseif($obj_detail->material_code == "0002-100006"){ //RT6P
                        $act_qty_rt6p+=$obj_detail->actual_qty;
                        $tar_qty_rt6p+=$obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("Z".$row, $act_qty_rt6p);
                        $excel->getActiveSheet()->setCellValue("AQ".$row, $tar_qty_rt6p);
                        
                    }
                    elseif($obj_detail->material_code == "0002-100409"){ //SK163
                        $act_qty_sk163+=$obj_detail->actual_qty;
                        $tar_qty_sk163+=$obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("AA".$row, $act_qty_sk163);
                        $excel->getActiveSheet()->setCellValue("AR".$row, $tar_qty_sk163);
                        
                    }
                    elseif($obj_detail->material_code == "0002-100410"){ //SK930
                        $act_qty_sk930+=$obj_detail->actual_qty;
                        $tar_qty_sk930+=$obj_detail->target_qty;
                        $excel->getActiveSheet()->setCellValue("AB".$row, $act_qty_sk930);
                        $excel->getActiveSheet()->setCellValue("AS".$row, $tar_qty_sk930);
                    ...
                    elseif($obj_detail->material_code == "0002-100402"){ //VIS 1003 
                        ...
                        $excel->getActiveSheet()->setCellValue("AC".$row, $act_qty_vis1003);
                        $excel->getActiveSheet()->setCellValue("AT".$row, $tar_qty_vis1003);
                    ...
                    elseif($obj_detail->material_code == "0002-100407"){ //VIS 1212R
                        ...
                        $excel->getActiveSheet()->setCellValue("AD".$row, $act_qty_vis1212r);
                        $excel->getActiveSheet()->setCellValue("AU".$row, $tar_qty_vis1212r);
                    ...
                    elseif($obj_detail->material_code == "0002-100416"){ //VIS 7080p
                        ...
                        $excel->getActiveSheet()->setCellValue("AE".$row, $act_qty_vis7080p);
                        $excel->getActiveSheet()->setCellValue("AV".$row, $tar_qty_vis7080p);
                ...
                */
                ...
                if($tar_qty_flobasrpf34 === 0){
                  //  $var_flobasrpf34 = 0;
                    ...
                    $excel->getActiveSheet()->setCellValue("BK".$row, round($var_flobasrpf34,2));
                }
                if($tar_qty_s523n === 0){
                    //$var_s523n = 0;
