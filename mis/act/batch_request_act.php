...
if(IS_AJAX){
	...
	$conn = mysqli_connect(DB_SERVER,DB_USER,DB_PASS);
        ...
        if($today == $pieces[0]){
            ...
            //$kueri =" SELECT SUM(distinct `batch_transaction`.`batch_vol`) AS total_load, SUM(distinct `batch_request`.`batch_vol`) AS volume_out FROM `batch_transaction` LEFT JOIN `batch_request` ON `batch_transaction`.`so_no` = `batch_request`.`so_no` WHERE `batch_request`.`so_no`='".$so_no."' AND `batch_request`.`product_code`='".$data['product_code']."' AND `batch_request`.`flag_code`!= 'D' AND `batch_request`.`batch_date`= '".$sched_date."'";

             //$kueri = "SELECT SUM(`batch_vol`) AS volume_total, SUM(`batch_vol`) AS volume_out FROM `batch_transaction`WHERE `so_no`='".$so_no."' AND `product_code` = '".$data['product_code']."' AND `flag_code` != 'D'";
            ...
            //$kueri = "SELECT SUM( `batch_transaction`.`batch_vol`) AS total_load, SUM(`batch_request`.`batch_vol`) AS volume_out FROM `batch_transaction` LEFT JOIN `batch_request` ON `batch_transaction`.`so_no` = `batch_request`.`so_no` WHERE `batch_transaction`.`so_no`='".$so_no."' AND `batch_request`.`product_code`='".$data['product_code']."' AND `batch_request`.`flag_code`!= 'D' AND `batch_request`.`batch_date`= '".$sched_date."'";

           

            $hasil = mysqli_query($conns,$kueri);

            $kueri1 = "SELECT SUM(`batch_vol`) AS volume_total FROM `batch_transaction`WHERE `so_no`='".$so_no."' AND `product_code` = '".$data['product_code']."' AND `flag_code` != 'D'";
             $hasil1 = mysqli_query($conns,$kueri1);
            ...
            if(mysqli_num_rows($hasil) > 0){
                ...
                $volume_total= $row1['volume_total'];
            ...
            $output = array(
                ...
                "delv_vol"          =>  $data['qty_so'],
                "today_b_request"   =>  $data['1-24hr'],
                'plant_id'          =>  $plant_id,
                'out_volume'        =>  $volume_out,
                'volume_total'      =>  $volume_total,
                'sisa_erp'          =>  $data['qty_so']-$volume_total,
        ...
        elseif($today - $pieces[0] == 1){
            ...
            if($thishour <= 7){
                ...
                $result_query_cek_schedule = mysql_query($query_cek_schedule);
                if(!$result_query_cek_schedule){
                    throw new Exception("<b>ERROR MYSQL</b> : ".mysql_error());
                }

                $jumlah_data = mysql_num_rows($result_query_cek_schedule);
