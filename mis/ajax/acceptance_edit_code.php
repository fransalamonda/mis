...
if(mysqli_num_rows($r_check) == 0){
    $output = array(
        "status"    =>  0,
        "msg"       =>  "Data dengan kode <b>".$_POST['code']."</b> tidak ditemukan"
    );
    exit(json_encode($output));
}
...
$q_update = "UPDATE  `tbl_code_acceptance` SET  "
        . "`id_acceptance` = '".$_POST['cate']."',  "
        . "`desc` = '".$_POST['desc']."',  "
...
$output = array(
    "status"    =>  1,
    "msg"       =>  "Kode Acceptance <b>".$_POST['code']."</b> baru berhasil diperbarui, halaman akan di-refresh"
);
exit(json_encode($output));
