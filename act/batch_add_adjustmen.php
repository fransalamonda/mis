...
if(IS_AJAX){
    try {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_POST['semen_c'] > 9999){ throw new Exception("Max Qty semen c 9999!"); }
            ...
            if(isset($_POST['so_no']) && !empty($_POST['so_no']) && isset($_POST['docket_no']) && !empty($_POST['docket_no']) && 
               ...
               isset($_POST['plast83am']) && isset($_POST['steelAGG1'])&& isset($_POST['gencb10']) && isset($_POST['flbrpf34']) && 
               isset($_POST['Bsand']) && isset($_POST['Vsand'])&& isset($_POST['vis5002']) && isset($_POST['genr8212']) && 
               isset($_POST['get702']) && isset($_POST['genb1714']) && isset($_POST['flbnf15'])&& isset($_POST['vis8097sv']) && 
                ...
                if(mysqli_num_rows($hasil_mix) == 0){
                    ...
                    if(!mysqli_query($conns,$insertMD)){throw new Exception('Error002 : Gagal Menambahkan mix_design,   '.mysqli_error());}
                             ...
                             $insertsemen_c = "INSERT INTO `mix_design_composition`(`mch_code`,`product_code`,`material_group`,`seq_no`,`material_code`,`material_name`,`mix_qty`,`mix_qty_adj`,`unit`,`flag_code`)"
                                        . "VALUES ('".PLANT_ID."','ADJUSTMENT','Cement','','0000-100021','SEMEN C','".$semen_c."','0','KG','I')";
							...
							$insertvis5002 = "INSERT INTO `mix_design_composition`(`mch_code`,    `product_code`,`material_group`,`seq_no`,`material_code`,`material_name`,`mix_qty`,`mix_qty_adj`,`unit`,`flag_code`)"
                                        . "VALUES('".PLANT_ID."','ADJUSTMENT',  'Admixture',         '',      '0002-100466',  'VIS 5002','".$vis5002."','0','KG','I')";
                            if(!mysqli_query($conns,$insertvis5002)){
                                    throw new Exception('Error020 : Gagal Menambahkan VIS 5002,   '.mysqli_error());	
								}
                            ...
                            if(!mysqli_query($conns,$insertvis8097sv)){
                                    throw new Exception('Error026 : Gagal Menambahkan vis8097sv,   '.mysqli_error());
                                }
